...
def get_sos_contacts(
    ...
    try:
        ...
        if not force_refresh:
            ...
            if cached_data:
                ...
                if is_cache_valid(cached_data, ttl_days=DEFAULT_TTL_DAYS):
                    logger.info(
                        f"Returning fresh cached SOS data", extra={"country": country}
                    )
                    return cached_data
                else:
                    logger.info(
                        f"Cache expired, fetching fresh data",
                        extra={"country": country},
                    )

        # Step 2: Fetch fresh data using SerpApi + LLM
        logger.info(f"Fetching fresh SOS contacts", extra={"country": country})
        ...
        if fresh_contacts:
            ...
            if cache_success:
                logger.info(f"Cached fresh SOS contacts", extra={"country": country})
            return fresh_contacts

        # Step 4: Fallback to cached data even if stale
        logger.warning(
            f"Could not fetch fresh data, attempting to use stale cache",
            extra={"country": country},
        )
        ...
        logger.error(
            f"Error in get_sos_contacts: {str(e)}",
            extra={"country": country},
            exc_info=True,
        )
...
def fetch_fresh_sos_contacts(country: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not search_results:
            logger.warning(f"No search results found", extra={"country": country})
            return None
        ...
        if not extracted_contacts:
            logger.warning(
                f"LLM failed to extract contacts", extra={"country": country}
            )
            return None

        # Step 3: Validate structure
        if not validate_contact_structure(extracted_contacts):
            logger.error(f"Invalid contact structure", extra={"country": country})
            return None
        ...
        logger.error(
            f"Error fetching fresh contacts: {str(e)}",
            extra={"country": country},
            exc_info=True,
        )
        return None
...
def search_emergency_contacts(country: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug(
            f"Retrieved {len(all_results)} search results", extra={"country": country}
        )
        return all_results

    except Exception as e:
        logger.error(
            f"Error in SerpApi search: {str(e)}",
            extra={"country": country},
            exc_info=True,
        )
        return []
...
def extract_contacts_with_llm(
    ...
    try:
        ...
        logger.error(
            f"LLM extraction error: {str(e)}", extra={"country": country}, exc_info=True
        )
        return None
...
def parse_llm_response(response_text: str, country: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(
            ...
            extra={"country": country, "response_preview": response_text[:200]},
        )
        return None
    except Exception as e:
        logger.error(
...
def is_cache_valid(cached_data: Dict[str, Any], ttl_days: int = 30) -> bool:
    ...
    try:
        ...
        if not cached_at:
            logger.debug("No cached_at field found in cached data")
            return False
        ...
        logger.debug(
            f"Cache age: {age.days} days (TTL: {ttl_days} days) - {'fresh' if is_valid else 'stale'}"
        )
        return is_valid

    except Exception as e:
        logger.error(f"Error validating cache: {str(e)}", exc_info=True)
        return False
...
def validate_contact_structure(contacts: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if "crisis_hotlines" not in contacts:
            logger.error("Missing crisis_hotlines array in contact data")
            return False
        ...
        if len(hotlines) != CONTACT_COUNT:
            logger.error(f"Expected {CONTACT_COUNT} contacts, got {len(hotlines)}")
            return False
        ...
        for idx, contact in enumerate(hotlines, 1):
            for field in required_fields:
                if field not in contact or not contact[field]:
                    logger.error(f"Contact {idx} missing field: {field}")
                    return False

        logger.debug(f"Contact structure valid ({CONTACT_COUNT} contacts)")
        return True
