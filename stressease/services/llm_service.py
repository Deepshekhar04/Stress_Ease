...
def create_conversation_chain(user_context: str, memory: ConversationBufferWindowMemory) -> ConversationChain:
    """
    Create a LangChain ConversationChain with memory and context.
    ...
    Args:
        ...
        memory (ConversationBufferWindowMemory): Conversation memory instance
...
def build_user_context(user_profile: Optional[Dict[str, Any]], mood_summary: Optional[str]) -> str:
    """
...
def generate_chat_response(conversation_chain: ConversationChain, user_message: str) -> str:
    """
    Generate chat response using the conversation chain (Chain B).
    
    Args:
        conversation_chain (ConversationChain): The conversation chain with memory
